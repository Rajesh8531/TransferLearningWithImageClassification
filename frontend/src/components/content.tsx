...
const Content = ({imageData,prediction}:{imageData:string,prediction:string}) => {

    const animalsWithClasses = {'antelope': 0, 'badger': 1, 'bat': 2, 'bear': 3, 'bee': 4, 'beetle': 5, 'bison': 6,
    ...
    const animals = Object.keys(animalsWithClasses)

  return (
    <div className="w-full flex flex-row gap-x-4">
        <Scrollbar items={animals} />
        <div className="space-y-8 w-full">
          {prediction && <Prediction prediction={prediction} imageData={imageData} />}
          <Description /> 
        </div>
    </div>
  )
